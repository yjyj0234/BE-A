...
### 미구현 항목
- 대기열(waitlist) 기능
- 현재는 수강 신청(PENDING) 시점에 정원을 선점하도록 구현하였습니다. 결제를 완료하지 않은 신청이 장시간 유지될 경우 실제 확정 수강 인원과 정원 점유 상태가 달라질 수 있습니다. 실제 서비스에서는 PENDING 상태의 유효 시간을 두고, 일정 시간 내 결제가 완료되지 않으면 자동 취소 처리하는 방식이 필요합니다.
